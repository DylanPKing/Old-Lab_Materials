...
void powerEntryFunction(const sparseMat, sparseMat&, int power);
void multMat(const sparseMat, sparseMat, sparseMat&);
...
int main(int argc, char *argv[])
{
  ...
  {
  	--power;
  	powerEntryFunction(rows, prod, power);
...
void powerEntryFunction(const sparseMat rows, sparseMat& prod, int power)
{
	if (power == 2)
		multMat(rows, rows, prod);
	else
	{
		powerEntryFunction(rows, prod, power / 2);
		multMat(prod, prod, prod);
	}
}
